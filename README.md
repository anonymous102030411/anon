...
### Table 3a - Model Worst Group Accuracy Performance (with attribute annotation):
...
| Dataset    | α=1e4   | α=5e4   | α=1e5   | α=5e5   | Mean  | STD  | 
|------------|-------|-------|-------|-------|-------|------|
...
Figure 1: Landbird Prototypes. Each row shows the top-10 validation samples closest to one of four learned prototypes from our Diversified Prototypical Ensemble (DPE) model on the Landbirds class of the Waterbirds dataset. ChatGPT was used to analyze each prototype’s semantic structure in a zero-shot setting. The results reveal that learned prototypes align with coherent visual or ecological subpopulations—e.g., small yellow songbirds across diverse backgrounds, upright-postured forest-edge dwellers, forest birds appearing in beach or human-present scenes (highlighting spurious correlations), and muted-tone songbirds in leafy, texture-poor vegetation. These interpretable clusters suggest that DPE discovers meaningful intra-class modes beyond spurious environmental cues.
![My Image](images/landbird_prototypes_with_chatgpt_descriptions.jpg)

Figure 2: Waterbird Prototypes. Each row depicts the top-10 validation samples closest to one of four prototypes learned for the Waterbirds class. Using ChatGPT for latent cluster interpretation, we observe that prototypes capture semantically rich subpopulations: divers and floaters in aquatic settings, large-bodied birds in dynamic flight poses, compact-bodied seabirds shown in terrestrial or bamboo-heavy scenes (an ecologically-confounded but visually coherent mode), and upright-standing gulls with consistent visual separation. These findings demonstrate that DPE induces structured prototype-subgroup alignment that meaningfully reflects bird morphology, pose, and context, supporting its robustness under subpopulation shift.
